...
local DefaultState = {
    ...
    WatermarkText = "PhuongThaoCutieee",
...
local function EnsureBoxESP()
    if not Drawing or not Drawing.new then
        return
    end
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and not BoxESPLines[plr] then
            BoxESPLines[plr] = {}
            for i = 1, 12 do
                local line = Drawing.new("Line")
                line.Visible = false
                line.Thickness = State.BoxESPThickness
                line.Color = State.BoxESPColor
                table.insert(BoxESPLines[plr], line)
            end
        end
    end
end
...
local function UpdateDrawing()
    ...
    if State.BoxESPEnabled and BoxESPLines and Camera then
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr ~= LocalPlayer then
                local lines = BoxESPLines[plr]
                if lines then
                    local char = plr.Character
                    local root = char and char:FindFirstChild("HumanoidRootPart")
                    local hum = char and char:FindFirstChildOfClass("Humanoid")
                    if root and hum then
                        ...
                        local head = char:FindFirstChild("Head")
                        local headSize = head and head.Size.Y or 1
                        local hipHeight = hum.HipHeight or 2
                        local height = (hipHeight * 2) + (headSize * 0.5) + size.Y
                        local width = size.X * 1.2
                        local depth = size.Z * 1.2
                        
                        -- Calculate 8 corners of the box
                        local corners = {
                            cf * CFrame.new(-width/2, -height/2, -depth/2).Position,
                            cf * CFrame.new(width/2, -height/2, -depth/2).Position,
                            cf * CFrame.new(width/2, -height/2, depth/2).Position,
                            cf * CFrame.new(-width/2, -height/2, depth/2).Position,
                            cf * CFrame.new(-width/2, height/2, -depth/2).Position,
                        ...
                        if allOnScreen and #screenCorners == 8 then
                            ...
                            for i = 1, 12 do
                                ...
                                lines[i].Color = State.BoxESPColor
                                lines[i].Thickness = State.BoxESPThickness
                            end
                        else
                            for i = 1, 12 do
...
local function RefreshDrawingLoop()
    local want = State.ShowFov or State.CrosshairEnabled or State.TracersEnabled or State.BoxESPEnabled
...
local function SetBoxESPEnabled(enabled)
    State.BoxESPEnabled = enabled
    if enabled and not (Drawing and Drawing.new) then
        State.BoxESPEnabled = false
        TrySetValue("BoxESPEnabled", false)
        Notify("Box ESP", "Drawing API missing.")
        return
    end
    RefreshDrawingLoop()
end
...
local function DrawSkeletonLine(fromPos, toPos, line)
    if not Camera or not fromPos or not toPos then
        return false
    end
    local fromScreen, fromOnScreen = Camera:WorldToViewportPoint(fromPos)
    local toScreen, toOnScreen = Camera:WorldToViewportPoint(toPos)
...
local function UpdateSkeletonESP()
    if not State.SkeletonEnabled or not Drawing or not Drawing.new then
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr == LocalPlayer then
            ...
            if char then
                ...
                local root = GetBonePosition(char, "HumanoidRootPart")
                local head = GetBonePosition(char, "Head")
                local leftArm = GetBonePosition(char, "LeftUpperArm")
                local rightArm = GetBonePosition(char, "RightUpperArm")
                local leftForearm = GetBonePosition(char, "LeftLowerArm")
                local rightForearm = GetBonePosition(char, "RightLowerArm")
                local leftHand = GetBonePosition(char, "LeftHand")
                local rightHand = GetBonePosition(char, "RightHand")
                local leftLeg = GetBonePosition(char, "LeftUpperLeg")
                local rightLeg = GetBonePosition(char, "RightUpperLeg")
                local leftCalf = GetBonePosition(char, "LeftLowerLeg")
                local rightCalf = GetBonePosition(char, "RightLowerLeg")
                local leftFoot = GetBonePosition(char, "LeftFoot")
                local rightFoot = GetBonePosition(char, "RightFoot")
                local torso = GetBonePosition(char, "UpperTorso")
                local lowerTorso = GetBonePosition(char, "LowerTorso")
                
                local lineIndex = 1
                
                -- Head to Torso
                if head and torso then
                    DrawSkeletonLine(head, torso, lines[lineIndex])
                    lineIndex = lineIndex + 1
                end
                
                -- Torso to Lower Torso
                if torso and lowerTorso then
                    DrawSkeletonLine(torso, lowerTorso, lines[lineIndex])
                    lineIndex = lineIndex + 1
                end
                
                -- Torso to Arms
                if torso and leftArm then
                    DrawSkeletonLine(torso, leftArm, lines[lineIndex])
                    lineIndex = lineIndex + 1
                end
                if torso and rightArm then
                    DrawSkeletonLine(torso, rightArm, lines[lineIndex])
                    lineIndex = lineIndex + 1
                end
                
                -- Arms
                if leftArm and leftForearm then
                    DrawSkeletonLine(leftArm, leftForearm, lines[lineIndex])
                    lineIndex = lineIndex + 1
                end
                if rightArm and rightForearm then
                    DrawSkeletonLine(rightArm, rightForearm, lines[lineIndex])
                    lineIndex = lineIndex + 1
                end
                if leftForearm and leftHand then
                    DrawSkeletonLine(leftForearm, leftHand, lines[lineIndex])
                    lineIndex = lineIndex + 1
                end
                if rightForearm and rightHand then
                    DrawSkeletonLine(rightForearm, rightHand, lines[lineIndex])
                    lineIndex = lineIndex + 1
                end
                
                -- Lower Torso to Legs
                if lowerTorso and leftLeg then
                    DrawSkeletonLine(lowerTorso, leftLeg, lines[lineIndex])
                    lineIndex = lineIndex + 1
                end
                if lowerTorso and rightLeg then
                    DrawSkeletonLine(lowerTorso, rightLeg, lines[lineIndex])
                    lineIndex = lineIndex + 1
                end
                
                -- Legs
                if leftLeg and leftCalf then
                    DrawSkeletonLine(leftLeg, leftCalf, lines[lineIndex])
                    lineIndex = lineIndex + 1
                end
                if rightLeg and rightCalf then
                    DrawSkeletonLine(rightLeg, rightCalf, lines[lineIndex])
                    lineIndex = lineIndex + 1
                end
                if leftCalf and leftFoot then
                    DrawSkeletonLine(leftCalf, leftFoot, lines[lineIndex])
                    lineIndex = lineIndex + 1
                end
                if rightCalf and rightFoot then
                    DrawSkeletonLine(rightCalf, rightFoot, lines[lineIndex])
                    lineIndex = lineIndex + 1
                end
                ...
                for i = 1, lineIndex - 1 do
                    if lines[i] then
                        lines[i].Color = State.SkeletonColor
                    end
                end
            end
        end
...
local function SetSkeletonEnabled(enabled)
    State.SkeletonEnabled = enabled
    if enabled then
        if not (Drawing and Drawing.new) then
            State.SkeletonEnabled = false
            TrySetValue("SkeletonEnabled", false)
            Notify("Skeleton ESP", "Drawing API missing.")
            return
        end
        BindConnection("SkeletonESP", RunService.RenderStepped:Connect(UpdateSkeletonESP))
    else
        UnbindConnection("SkeletonESP")
        for _, lines in pairs(SkeletonLines) do
            for _, line in pairs(lines) do
                if line then
                    line.Visible = false
...
local function ApplyAllState()
    ...
    SetSkeletonEnabled(State.SkeletonEnabled)
    SetBoxESPEnabled(State.BoxESPEnabled)
...
local Window = WindUI:CreateWindow({
    Title = "PhuongThaoCutieee",
    Folder = "GiaHuy_Universal",
    Icon = "globe-lock",
    NewElements = true,
...
do
    local BoxESPSection = VisualTab:Section({ Title = "Box ESP", Opened = true })
    RegisterRef("BoxESPEnabled", BoxESPSection:Toggle({
        Title = "Enable Box ESP",
        Value = State.BoxESPEnabled,
        Callback = function(v)
            SetBoxESPEnabled(v)
        end
    }))
    RegisterRef("BoxESPColor", BoxESPSection:Colorpicker({
        Title = "Box Color",
        Default = State.BoxESPColor,
        Callback = function(v)
            State.BoxESPColor = v
            for _, lines in pairs(BoxESPLines) do
                for _, line in ipairs(lines) do
    ...
    RegisterRef("BoxESPThickness", BoxESPSection:Slider({
        Title = "Box Thickness",
        Step = 1,
        Value = {
            Min = 1,
            Max = 5,
            Default = State.BoxESPThickness,
        },
        Callback = function(v)
            State.BoxESPThickness = v
            for _, lines in pairs(BoxESPLines) do
                for _, line in ipairs(lines) do
                    if line then
                        line.Thickness = v
...
do
    local SkeletonSection = VisualTab:Section({ Title = "Skeleton ESP", Opened = true })
    RegisterRef("SkeletonEnabled", SkeletonSection:Toggle({
    ...
    RegisterRef("SkeletonColor", SkeletonSection:Colorpicker({
        Title = "Skeleton Color",
        Default = State.SkeletonColor,
        Callback = function(v)
            State.SkeletonColor = v
            for _, lines in pairs(SkeletonLines) do
                for _, line in pairs(lines) do
                    if line then
                        line.Color = v
    ...
    RegisterRef("SkeletonThickness", SkeletonSection:Slider({
        Title = "Skeleton Thickness",
        Step = 1,
        Value = {
            Min = 1,
            Max = 5,
            Default = State.SkeletonThickness,
        },
        Callback = function(v)
            State.SkeletonThickness = v
            for _, lines in pairs(SkeletonLines) do
                for _, line in pairs(lines) do
                    if line then
                        line.Thickness = v
...
local WatermarkRGBConnection = nil

-- RGB Rainbow Effect (HSL to RGB conversion for smooth rainbow)
...
local function StartWatermarkRGB()
    if WatermarkRGBConnection then
        WatermarkRGBConnection:Disconnect()
        WatermarkRGBConnection = nil
    end
    
    if not State.WatermarkEnabled or not State.WatermarkRGB or not WatermarkLabel then
        return
    end
    
    local rgbTime = 0
    local rgbSpeed = 0.8 -- Speed of color transition (higher = faster)
    
    WatermarkRGBConnection = RunService.RenderStepped:Connect(function(deltaTime)
        if not State.WatermarkEnabled or not State.WatermarkRGB or not WatermarkLabel then
            if WatermarkRGBConnection then
                WatermarkRGBConnection:Disconnect()
                WatermarkRGBConnection = nil
        ...
        rgbTime = rgbTime + deltaTime * rgbSpeed
        
        -- Create smooth rainbow effect (360 degrees hue cycle)
        local hue = (rgbTime * 60) % 360
        local saturation = 1
        local lightness = 0.5
        
        local rainbowColor = HSLToRGB(hue, saturation, lightness)
        WatermarkLabel.TextColor3 = rainbowColor
    end)
end

local function StopWatermarkRGB()
    if WatermarkRGBConnection then
        WatermarkRGBConnection:Disconnect()
        WatermarkRGBConnection = nil
    end
...
local function UpdateWatermark()
    if not WatermarkGui then
        ...
        WatermarkGui.Name = "GiaHuyWatermark"
        ...
        WatermarkLabel.Size = UDim2.new(0, 0, 0, 0)
        WatermarkLabel.Position = UDim2.new(0, 10, 0, 10)
        WatermarkLabel.BackgroundTransparency = 1
        WatermarkLabel.TextColor3 = State.WatermarkColor or Color3.fromRGB(255, 255, 255)
    ...
    if State.WatermarkEnabled then
        WatermarkLabel.Text = State.WatermarkText or "PhuongThaoCutieee"
        WatermarkLabel.Visible = true
        WatermarkGui.Enabled = true
        
        -- Update color based on RGB mode
        if State.WatermarkRGB then
            StartWatermarkRGB()
        else
            StopWatermarkRGB()
            WatermarkLabel.TextColor3 = State.WatermarkColor or Color3.fromRGB(255, 255, 255)
...
do
    local WatermarkSection = SettingsTab:Section({ Title = "Watermark", Opened = true })
    RegisterRef("WatermarkEnabled", WatermarkSection:Toggle({
    ...
    RegisterRef("WatermarkRGB", WatermarkSection:Toggle({
        Title = "RGB Rainbow Mode",
        Value = State.WatermarkRGB,
        Callback = function(v)
            State.WatermarkRGB = v
            UpdateWatermark()
        end
    }))
    RegisterRef("WatermarkColor", WatermarkSection:Colorpicker({
        Title = "Watermark Color (Static)",
        Default = State.WatermarkColor,
        Callback = function(v)
            State.WatermarkColor = v
            if not State.WatermarkRGB then
                UpdateWatermark()
            end
        end
    }))
...
Players.PlayerRemoving:Connect(function(plr)
    ...
    if SkeletonLines and SkeletonLines[plr] then
        for _, line in pairs(SkeletonLines[plr]) do
            if line then
                line:Remove()
            end
        end
    ...
    if BoxESPLines and BoxESPLines[plr] then
        for _, line in ipairs(BoxESPLines[plr]) do
            if line then
                line:Remove()
            end
        end
...
RefreshDrawingLoop()
UpdateWatermark()
Notify("PhuongThaoCutieee", "Loaded and ready.")